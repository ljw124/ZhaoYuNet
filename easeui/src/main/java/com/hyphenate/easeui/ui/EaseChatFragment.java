...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected int[] itemStrings = { R.string.attach_take_pic, R.string.attach_picture, R.string.attach_location, R.string.attach_voice};
    protected int[] itemdrawables = { R.drawable.ease_chat_takepic_selector, R.drawable.ease_chat_image_selector,
            R.drawable.ease_chat_location_selector, R.drawable.ease_chat_voice_call_receive};
    protected int[] itemIds = { ITEM_TAKE_PICTURE, ITEM_PICTURE, ITEM_LOCATION, ITEM_VOICE};
    ...
    protected void initView() {
        ...
        tvPhone = getView().findViewById(R.id.tv_phone);
        tvWechat = getView().findViewById(R.id.tv_wechat);
        tvPhone.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(getContext(), "互换手机号", Toast.LENGTH_SHORT).show();
        ...
        tvWechat.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Toast.makeText(getContext(), "互换微信号", Toast.LENGTH_SHORT).show();
                sendCMDMessage("wechat");
            }
        });
...
//        messageList.setAvatarShape(1);
        ...
        listView.addHeaderView(LayoutInflater.from(getContext()).inflate(R.layout.ease_chat_header, null));
            ...
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (ACTION_TYPING_BEGIN.equals(body.action()) && msg.getFrom().equals(toChatUsername)) {
                        ...
                        titleBar.setTitle(toChatUsername);
                    } else if ("phone".equals(body.action()) && msg.getFrom().equals(toChatUsername)) {
                        EMMessage message = EMMessage.createReceiveMessage(EMMessage.Type.TXT);
                        message.setAttribute(EaseConstant.MESSAGE_CUSTOM_CHAT_VIEW, true);
                        message.setAttribute("type", "phone");
                        message.setAttribute("from", msg.getFrom());
                        message.addBody(new EMTextMessageBody("对方请求交换手机号，您是否同意"));
//                        EMClient.getInstance().chatManager().saveMessage(message);
                        EMClient.getInstance().chatManager().getConversation(msg.getFrom(), EMConversation.EMConversationType.Chat, true).appendMessage(message);
                        messageList.refreshSelectLast();
                    } else if ("wechat".equals(body.action()) && msg.getFrom().equals(toChatUsername)) {
                        EMMessage message = EMMessage.createReceiveMessage(EMMessage.Type.TXT);
                        message.setAttribute(EaseConstant.MESSAGE_CUSTOM_CHAT_VIEW, true);
                        message.setAttribute("type", "wechat");
                        message.setAttribute("from", msg.getFrom());
                        message.addBody(new EMTextMessageBody("对方请求交换微信号，您是否同意"));
//                        EMClient.getInstance().chatManager().saveMessage(message);
                        EMClient.getInstance().chatManager().getConversation(msg.getFrom(), EMConversation.EMConversationType.Chat, true).appendMessage(message);
                        messageList.refreshSelectLast();
                    } else if ("PhoneYES".equals(body.action()) && msg.getFrom().equals(toChatUsername)) {
                        sendOwnMessage(msg.getTo() + "的手机号是：" + "13333333333");
                    } else if ("WechatYes".equals(body.action()) && msg.getFrom().equals(toChatUsername)) {
                        sendOwnMessage(msg.getTo() + "的微信号是：" + "1555555555");
                    }else if ("PhoneNO".equals(body.action()) && msg.getFrom().equals(toChatUsername)) {
                        sendOwnMessage(msg.getTo() + "拒绝了交换手机号的请求");
                    }else if ("WechatNO".equals(body.action()) && msg.getFrom().equals(toChatUsername)) {
                        sendOwnMessage(msg.getTo() + "拒绝了交换微信号的请求");
                    }
                }
            });
        }
